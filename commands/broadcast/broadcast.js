...
const broadcastCommand = new SlashCommandBuilder()
    .setName('broadcast')
    .setDescription('Envoyer une annonce en message privé à tous les B2. (Attention aux abus)');

module.exports = {
    data: broadcastCommand,
    async execute(interaction) {
        return;
        if (!member.roles.cache.has("1288402672042639361") || !member.roles.cache.has("1148553469448421407")) {
            await interaction.reply({ content: 'Vous devez être de l administration ou un délégué pour utiliser cette commande.', ephemeral: true });
            return;
        }
            ...
            try {
                ...
                const modal = new ModalBuilder()
                    ...
                    .setTitle('Envoyer un message privé à touts ceux qui ont le rôle Populasse. Attention aux abus.');
